...
**Note on scope of `Deliverer.deliver()` vs TRD-v3 §3.7's signature.** TRD-v3 shows `deliver(*, run_id, issue, worktree_path, branch, scores)`. L0 has no `issue` (no queue yet — that's L2) and no loop-computed `scores` summary. This TRS's `deliver()` takes `title`/`body` instead — plain strings the *caller* composes (from run outcome + whatever it has on hand). Phase L2 is expected to widen the call site to build `title`/`body` from `issue` + `scores`; the `Deliverer` Protocol itself does not need to change shape for that — L2 just constructs a richer `body` string. Flagged as a **Pending Decision** (see below) in case the maintainer prefers to lock the final signature now.
...
* **[T-L0.3] Fix or xfail the content-pipeline drift test** [Effort: S–M]
  - **Description**: Root-cause `test_score_jobs_adapter_real_import_success`'s `AttributeError: module 'application.use_cases' has no attribute 'score_jobs'` — this means content-pipeline (the sibling repo) renamed or moved the `score_jobs` use case since atlas's adapter code was written. Two paths: (a) if the fix is a one-line import-path update in atlas's `library_adapters` code, fix it directly; (b) if it requires a content-pipeline-side change atlas cannot make from this repo, mark the test `@pytest.mark.xfail(reason="content-pipeline moved score_jobs; tracked in BACKLOG.md", strict=False)` and add a `BACKLOG.md` entry. **Do not silently skip or delete the test** — TRD-v3 §14's "fix or xfail... so a green suite means green" is explicit that the drift must be visible, not hidden.
  - **Acceptance Criteria**:
      - [ ] `pytest tests/ -q` exits 0 (green) — either because the test now passes, or because it's `xfail`-marked and thus doesn't count as a failure
      - [ ] If `xfail`-marked: a `BACKLOG.md` entry links to the specific content-pipeline symbol that moved
  - **Files to Create/Modify**:
      - `tests/integration/test_job_adapters_real_import.py` — fix or `xfail` mark
      - `src/atlas/library_adapters/*` (only if the root cause is fixable from atlas's side — file TBD pending investigation)
      - `docs/1_product_and_research/BACKLOG.md` — tracking entry if `xfail`
  - **Dependencies**: T-L0.1
  - **Testing Requirements**: the fix/mark itself is validated by the full suite going green
...
1. **`Deliverer.deliver()` signature — narrow (this TRS) vs. wide (TRD-v3 §3.7 literal).** This TRS defines `deliver(*, run_id, branch, worktree_path, title, body)` since L0 has no `issue`/`scores` objects yet (those arrive in L2). TRD-v3 §3.7 shows `deliver(*, run_id, issue, worktree_path, branch, scores)`. **Options:** (a) build the narrow signature now, widen in L2 when `issue`/`scores` exist (recommended — YAGNI, and L2 owns those types anyway); (b) define `issue`/`scores` as loosely-typed placeholders now (e.g. `dict | None`) so the Protocol shape never changes. Recommend (a).
...
3. **Fix vs. `xfail` the content-pipeline drift test (T-L0.3).** Depends on whether the `score_jobs` rename is a one-line fix reachable from atlas's side or a content-pipeline-side change outside this repo's control. **Options:** (a) investigate content-pipeline's actual current module layout first (requires access to that sibling repo) and fix directly if trivial; (b) default to `xfail` + BACKLOG entry now, revisit later. Recommend investigating first (a) since "fix" is explicitly preferred over "xfail" per TRD-v3 §14's own phrasing ("fix or xfail"), but xfail is the acceptable fallback.
4. **Should atlas's own repo get a starter `.claude/settings.json` allowlist in L0?** TRD-v3 §7 specifies this file lives in the *target repo* (user-authored). Since v3.0's first target repo is atlas itself (TRD-v3 §7 config example: `repos = ["anant-gupta-utexas/atlas"]`), L0 could pre-author a sane starter allowlist now, or leave it for whoever first runs loop mode (L2) to author. **Options:** (a) author it now as part of T-L0.4/T-L0.6 groundwork; (b) defer entirely to L2 (recommended — the allowlist's actual required tool set is easier to determine once `loop_dev.yaml` (L1) and the loop's actual prompt shape (L2) exist; authoring it blind in L0 risks getting it wrong twice).
